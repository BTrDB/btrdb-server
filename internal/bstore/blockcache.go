...
func (bs *BlockStore) walkCache() {
	...
	for {
		...
		fw++
		if it.older == nil {
			lg.Infof("fw walked to end, compare %p/%p", it, bs.cacheold)
		}
		it = it.older
	}
	it = bs.cacheold
	for {
		...
		bw++
		if it.newer == nil {
			lg.Infof("bw walked to end, compare %p/%p", it, bs.cachenew)
		}
		it = it.newer
	}
	lg.Infof("Walked cache fw=%v, bw=%v, map=%v", fw, bw, len(bs.cachemap))
}
