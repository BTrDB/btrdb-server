...
func TestInsertAndSingleRangeRelease(t *testing.T) {
	...
	err = jp.ReleaseJournalEntries(ctx, nn, cp, &fullrange)
	require.NoError(t, err)
	...
	if found {
		t.Fatalf("No readback records should have been found")
	}
}
...
func TestInsertAndMultiRangeRelease(t *testing.T) {
	...
	err = jp.ReleaseJournalEntries(ctx, nn, cp, &configprovider.MashRange{Start: rA, End: rC})
	require.NoError(t, err)
	require.Equal(t, true, hasEntries())

	err = jp.ReleaseJournalEntries(ctx, nn, cp, &configprovider.MashRange{Start: rD, End: rE})
	require.NoError(t, err)
	require.Equal(t, true, hasEntries())
